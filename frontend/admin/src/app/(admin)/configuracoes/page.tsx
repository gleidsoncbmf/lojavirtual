...
export default function SettingsPage() {
            ...
            .then((data) => {
                ...
                if (data.theme) {
                    setThemeForm({
                        ...
                        seo_title: data.theme.seo_title || '',
                        seo_description: data.theme.seo_description || '',
                    });
                }
                setPaymentForm(data.payment_config || {});
                setWhatsappForm(data.whatsapp || '');
                if (data.domains?.[0]) {
                    setDomainForm(data.domains[0].domain);
                }
            })
            .catch(console.error)
    ...
    const handleSaveStore = async (e: React.FormEvent) => {
        ...
        setSaving(true);
        try {
            const updated = await updateStoreInfo(storeForm);
            setSettings(updated);
    ...
    const handleSavePayment = async (e: React.FormEvent) => {
        ...
        try {
            ...
            showSuccess('Configuração de pagamento atualizada!');
        } catch { /* ignore */ } finally { setSaving(false); }
    };
    ...
    const handleSaveWhatsApp = async (e: React.FormEvent) => {
        ...
        setSaving(true);
        try {
            await updateWhatsApp(whatsappForm);
    ...
    const handleSaveDomain = async (e: React.FormEvent) => {
        ...
        setSaving(true);
        try {
            await updateDomain(domainForm);
            showSuccess('Domínio configurado!');
        } catch { /* ignore */ } finally { setSaving(false); }
    };
    ...
    return (
        <div className="space-y-6 animate-in">
            ...
            {/* Tabs */}
            <div className="flex gap-1 bg-[#0f111a] border border-white/5 rounded-xl p-1 overflow-x-auto">
                {tabs.map((tab) => {
                    const Icon = tab.icon;
                    return (
                        <button
            ...
            <div className="bg-[#0f111a] border border-white/5 rounded-2xl p-6">
                ...
                {activeTab === 'store' && (
                    <form onSubmit={handleSaveStore} className="space-y-5 max-w-xl">
                        <h2 className="text-lg font-semibold text-white mb-4">Informações da Loja</h2>
                        <div>
                            <label htmlFor="store-name">Nome da Loja</label>
                            <input id="store-name" type="text" value={storeForm.name} onChange={(e) => setStoreForm({ ...storeForm, name: e.target.value })} />
                        </div>
                        <div>
                ...
                {activeTab === 'theme' && (
                    <form onSubmit={handleSaveTheme} className="space-y-5 max-w-xl">
                        <h2 className="text-lg font-semibold text-white mb-4">Personalização do Tema</h2>
                            ...
                            {[
                                ...
                                <div key={key}>
                                    ...
                                    <div className="flex items-center gap-3">
                                        <input
                                            type="color"
                                            value={(themeForm as Record<string, string>)[key] || '#000000'}
                                        ...
                                        <input
                                            id={key}
                                            type="text"
                                            value={(themeForm as Record<string, string>)[key] || ''}
                                            onChange={(e) => setThemeForm({ ...themeForm, [key]: e.target.value })}
                                            className="flex-1"
                                            maxLength={7}
                                        />
                        ...
                        <div>
                            ...
                            <input id="seo_title" type="text" value={themeForm.seo_title || ''} onChange={(e) => setThemeForm({ ...themeForm, seo_title: e.target.value })} placeholder="Título da loja nos buscadores" />
                        </div>
                        <div>
                            <label htmlFor="seo_description">SEO — Descrição</label>
                            <textarea id="seo_description" rows={3} value={themeForm.seo_description || ''} onChange={(e) => setThemeForm({ ...themeForm, seo_description: e.target.value })} placeholder="Descrição para mecanismos de busca..." />
                        </div>
                        <SaveButton saving={saving} />
                    </form>
                )}
                ...
                {activeTab === 'payments' && (
                    <form onSubmit={handleSavePayment} className="space-y-6 max-w-xl">
                        <h2 className="text-lg font-semibold text-white mb-4">Gateways de Pagamento</h2>
                        ...
                        <div className="bg-white/[0.02] border border-white/5 rounded-xl p-5 space-y-4">
                            <div className="flex items-center justify-between">
                                ...
                                <label className="flex items-center gap-2 cursor-pointer">
                                    <input
                                        type="checkbox"
                                        checked={paymentForm.stripe?.enabled || false}
                                        onChange={(e) => setPaymentForm({
                                            ...paymentForm,
                                            stripe: { ...paymentForm.stripe, enabled: e.target.checked } as PaymentConfig['stripe'],
                                        })}
                            ...
                            <div>
                                ...
                                <input type="text" value={paymentForm.stripe?.public_key || ''} onChange={(e) => setPaymentForm({ ...paymentForm, stripe: { ...paymentForm.stripe, public_key: e.target.value, enabled: paymentForm.stripe?.enabled || false } })} placeholder="pk_..." />
                            </div>
                            <div>
                                <label>Secret Key</label>
                                <input type="password" value={paymentForm.stripe?.secret_key || ''} onChange={(e) => setPaymentForm({ ...paymentForm, stripe: { ...paymentForm.stripe, secret_key: e.target.value, enabled: paymentForm.stripe?.enabled || false } })} placeholder="sk_..." />
                            </div>
                            <div>
                                <label>Webhook Secret</label>
                                <input type="password" value={paymentForm.stripe?.webhook_secret || ''} onChange={(e) => setPaymentForm({ ...paymentForm, stripe: { ...paymentForm.stripe, webhook_secret: e.target.value, enabled: paymentForm.stripe?.enabled || false } })} placeholder="whsec_..." />
                            </div>
                        </div>
                        ...
                        <div className="bg-white/[0.02] border border-white/5 rounded-xl p-5 space-y-4">
                            <div className="flex items-center justify-between">
                                ...
                                <label className="flex items-center gap-2 cursor-pointer">
                                    <input
                                        type="checkbox"
                                        checked={paymentForm.mercadopago?.enabled || false}
                                        onChange={(e) => setPaymentForm({
                                            ...paymentForm,
                                            mercadopago: { ...paymentForm.mercadopago, enabled: e.target.checked } as PaymentConfig['mercadopago'],
                                        })}
                                        className="w-4 h-4 rounded accent-indigo-500"
                                    />
                                    <span className="text-sm text-gray-400">Ativo</span>
                            ...
                            <div>
                                ...
                                <input type="password" value={paymentForm.mercadopago?.access_token || ''} onChange={(e) => setPaymentForm({ ...paymentForm, mercadopago: { ...paymentForm.mercadopago, access_token: e.target.value, enabled: paymentForm.mercadopago?.enabled || false } })} placeholder="APP_USR-..." />
                            </div>
                            <div>
                                <label>Public Key</label>
                                <input type="text" value={paymentForm.mercadopago?.public_key || ''} onChange={(e) => setPaymentForm({ ...paymentForm, mercadopago: { ...paymentForm.mercadopago, public_key: e.target.value, enabled: paymentForm.mercadopago?.enabled || false } })} placeholder="APP_USR-..." />
                            </div>
                        </div>
                        <SaveButton saving={saving} />
                    </form>
                ...
                {activeTab === 'whatsapp' && (
                    <form onSubmit={handleSaveWhatsApp} className="space-y-5 max-w-xl">
                        <h2 className="text-lg font-semibold text-white mb-4">WhatsApp</h2>
                        <div>
                            ...
                            />
                            <p className="text-gray-600 text-xs mt-1.5">Formato: código do país + DDD + número (sem espaços ou hífens)</p>
                        </div>
                        <SaveButton saving={saving} />
                    </form>
                )}
                ...
                {activeTab === 'domain' && (
                    <form onSubmit={handleSaveDomain} className="space-y-5 max-w-xl">
                        <h2 className="text-lg font-semibold text-white mb-4">Domínio Próprio</h2>
                        <div>
                            ...
                            <p className="text-gray-600 text-xs mt-1.5">
                                Após configurar, aponte o DNS do seu domínio para o IP da plataforma.
                            </p>
                        </div>
                        {settings?.domains?.[0] && (
                            <div className="flex items-center gap-2 text-sm">
                                <span className={`w-2 h-2 rounded-full ${settings.domains[0].verified ? 'bg-emerald-400' : 'bg-amber-400 animate-pulse'}`} />
                                <span className={settings.domains[0].verified ? 'text-emerald-400' : 'text-amber-400'}>
